...
> **⚠️ บันทึกย้อนหลังของเซสชันหนึ่ง ไม่ใช่แหล่งอ้างอิงสถานะ — ใช้ `docs/FEATURE-MAP-AND-BACKLOG.md`**
>
> ไฟล์นี้บันทึกว่า "ตั้งใจทำอะไร" ซึ่งหลายข้อเขียนว่าเสร็จแล้วแต่ตรวจโค้ดพบว่ายังต่อปุ่มไม่ครบ
> (เช่น `deleteAgendaItem` มีจริงแต่ไม่มีใครเรียก, helper วันที่ local เขียนแล้วแต่จุดเรียกยังใช้ UTC)
>
> **จุดที่ผิดและแก้แล้ว:** บรรทัดที่เขียนว่า "สวิตช์ใน `/tasks` = สถานะ done/open" — ไม่จริง
> สวิตช์นั้นคือปุ่ม**แชร์กับครอบครัว** (`is_shared`) ส่วน done/open เป็น Button ที่อยู่ข้าง ๆ
...
- สวิตช์ใน `/tasks` = **แชร์กับครอบครัว** (`is_shared`) ไม่ใช่ done/open — done/open คือ Button ข้าง ๆ
