...
> **⚠️ เอกสารนี้ไม่ใช่แหล่งอ้างอิงสถานะแล้ว — ใช้ `docs/FEATURE-MAP-AND-BACKLOG.md`**
>
> ตารางลำดับงานและ checkbox ข้างล่างค้างอยู่ที่ราวต้นเดือน ก.ย. หลายข้อที่ยังเขียนว่า
> "เหลือ" (เช่น payment/escrow) ทำเสร็จไปแล้ว การมีเอกสาร 2 ชุดที่เล่าคนละเรื่องคือ
> สาเหตุที่รอบก่อนสับสนว่าอะไรเสร็จอะไรไม่เสร็จ
>
> **สิ่งที่ยังใช้ได้จากไฟล์นี้:** รายละเอียดดีไซน์ สเปก schema และเหตุผลเชิงสถาปัตยกรรม
> ของแต่ละขั้น ซึ่งไม่ได้ย้ายไปไว้ที่อื่น
>
> **สิ่งที่อย่าเชื่อจากไฟล์นี้:** สถานะว่าอะไรทำแล้ว/ยังไม่ทำ
...
- [ ] **LIFF สำหรับปุ่มบนการ์ด LINE** (จาก 1.3/4b) — **ทบทวนก่อนทำ**: ตอนนี้ปุ่มเปิดเบราว์เซอร์หลักของเครื่อง (`openExternalBrowser=1`) ตามที่เจ้าของขอ · LIFF เปิดในแอป LINE ซึ่งเป็นสิ่งที่ตัดสินใจเลี่ยงไปแล้ว และเอกสารบอกว่า `openExternalBrowser` ไม่มีผลกับ LIFF · ถ้ายังต้องการ: LINE Login channel → LIFF → Add (Full, Endpoint `https://lavieos.netlify.app`, Scope `openid`) → env `LINE_LIFF_ID` (โค้ดรองรับแล้ว)
...
# 1.9 แนบไฟล์ ✅ 2026-09-19 `47015f7`
...
- [x] ปุ่มบนการ์ด — `AttachmentControl` ใน Money (รายจ่าย+รายรับ) และ Tasks: ไม่มีไฟล์ → "แนบใบเสร็จ" · `kind='attachment'` → thumbnail (รูป, signed URL 5 นาที) หรือ chip ชื่อไฟล์ กดเปิด + เปลี่ยน (ลิงก์ใหม่ก่อน แล้วลบไฟล์+แถวเก่า) + ลบ (มี dialog ยืนยัน: unlink → ลบไฟล์ → ลบแถว) · `kind='analyzed'` → chip "เอกสารต้นทาง" เปิดไฟล์ในคลัง ไม่มีปุ่มลบ/เปลี่ยน (คลังเป็นเจ้าของ)
- [x] "มาจากเอกสาร X" — chip "เอกสารต้นทาง" ข้างบนคือสิ่งนี้ ลิงก์ที่ AI เขียนไว้มองเห็นได้แล้วทั้งสองทิศ
- [x] attachment ไม่ปนกับเอกสารปกติ: หน้า Docs, นับเอกสารใน Today, รายการแชร์ใน Family และ **context ของ AI** (`phum.server.ts`) กรอง `kind='analyzed'` ทั้งหมด
- [x] tick ขั้น `orphan_attachments_deleted`: attachment ที่ไม่มีแถวไหนชี้ถึงเกิน 1 วัน (เช่นลบรายจ่ายทาง SQL) → ลบไฟล์ก่อนแล้วลบแถว — กันกรณี FK ชี้จากแถว→เอกสาร (SET NULL) ซึ่งไม่ลบเอกสารตามเมื่อแถวหาย
- [x] ทดสอบ (บัญชีชั่วคราว ลบแล้ว): แนบ PNG บนรายจ่าย → thumbnail + แถว/ไฟล์/ลิงก์ครบ → เปลี่ยนเป็น PDF → ไฟล์+แถวเก่าหาย → ลบ → ลิงก์ null ไฟล์+แถวหาย · แนบบน reminder · รายจ่ายที่ AI สร้างแสดง "เอกสารต้นทาง" · Docs แสดงเฉพาะเอกสารคลัง · orphan 2 วันถูกลบ, orphan ใหม่และไฟล์ที่ลิงก์อยู่ไม่ถูกแตะ · invariant ไฟล์↔แถว 1:1 ทุกขั้น
- [ ] ยังไม่มี: หน้า Today ยังไม่แสดงไฟล์แนบ (แสดงเฉพาะ Money/Tasks) · ลบรายจ่าย/รายรับจาก UI ยังไม่มีปุ่ม — ถ้าเพิ่มต้องลบ attachment ก่อน (หรือปล่อยให้ tick เก็บใน 1 วัน)
