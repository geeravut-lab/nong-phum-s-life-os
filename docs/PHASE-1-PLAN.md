...
- [ ] Route `/admin` ต้องมี guard ทั้ง**ฝั่ง server** และฝั่ง client (ซ่อนเมนู)
- [ ] guard ฝั่ง server **ต่อยอดจาก `requireSupabaseAuth`** ใน `src/integrations/supabase/auth-middleware.ts` ที่มีอยู่แล้ว — middleware ตัวนี้ให้ `context.userId` และ `context.supabase` (client ที่ผูก JWT ผู้ใช้ อยู่ใต้ RLS) และ server function ทั้ง 7 ตัวใช้อยู่แล้ว งานคือต่อ middleware อีกชั้นที่เช็ค `has_role` ไม่ใช่สร้างโครงสร้างใหม่
- [ ] ลบคอมเมนต์ `// This file is automatically generated. Do not edit it directly.` ที่หัวไฟล์ `auth-middleware.ts` — เศษจาก Lovable ที่ขั้น 2.6 ตกหล่น และขั้นนี้ต้องแก้ไฟล์นี้พอดี
...
- [ ] RLS: ใช้ pattern เดียวกับ `platform_settings` จริง ๆ ไม่ใช้ service role —
      `GRANT SELECT TO authenticated` + policy `SELECT ... USING (true)`
      และ policy เขียน `FOR ALL TO authenticated USING (has_role(auth.uid(),'admin')) WITH CHECK (has_role(auth.uid(),'admin'))`
...
> **ผลข้างเคียงที่ต้องรู้ก่อนลงมือ — ฟังก์ชันจะกลายเป็น async**
>
> ตอนนี้ `resolveProvider()` และ `getModel()` เป็น sync เพราะอ่านแค่ `process.env`
> พออ่าน DB ก่อน ทั้งคู่ต้องเป็น async ซึ่งลามขึ้นไปถึง `withProviderFallback()`
> และ **call site 5 จุด** — `phum.server.ts` 3 จุด + `marketplace.server.ts` 2 จุด
> ทำได้ไม่ยาก แต่ให้นับเป็นการแก้ 6 ไฟล์ ไม่ใช่จุดเดียว

- [ ] `assertAiProviderConfig()` ตอน boot **เปลี่ยนความหมาย** เป็นการตรวจ env พื้นฐานเท่านั้น
      (env ที่ตั้งไว้ชี้ไป provider ที่มี key จริงไหม) — มันไม่การันตีสิ่งที่รันจริงอีกต่อไป เพราะค่าใน DB override ทีหลังได้
      เขียนคอมเมนต์กำกับให้ชัด ไม่งั้นคนอ่านทีหลังจะเข้าใจผิดว่า boot ผ่าน = ใช้งานได้แน่
- [ ] เพิ่ม **runtime guard**: ตอนหยิบค่าจาก DB มาใช้ ถ้า provider นั้นไม่มี key ใน env ให้**ถอยไปใช้ค่าจาก env แทน** (ไม่ใช่โยน error ใส่ผู้ใช้) แล้วเขียนแถวลง `ai_events` เป็น `error` พร้อม `error_code` บอกว่าขาด env ตัวไหน
      — กันกรณีที่ผ่าน guard ตอน save มาแล้วแต่ env ถูกถอดออกทีหลัง
