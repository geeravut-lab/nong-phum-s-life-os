...
- [ ] 4 — LINE: (พี่) สร้าง LINE Login channel + link OA + token ตามรายการข้างบน · (ที่นี่) migration `line_links`/`line_link_states` → routes `/line/connect` + `/line/callback` + ปุ่มใน Settings → Flex 2 แบบ → push + profile check + quota guard + retry · ทดสอบเครื่องจริง iOS/Android
