...
export const getBillingPublic = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: prof } = await (
      ...
      .maybeSingle();
    const planTier = (prof?.plan_tier as string) || "free";
    const planExpiresAt = (prof?.plan_expires_at as string | null) ?? null;
    return {
      settings,
      usage: usage ?? {
        chat_count: 0,
      ...
      yearMonth: ym,
      isPremium: premium,
      planTier: premium ? (planTier === "family" ? "family" : "premium") : "free",
      planExpiresAt,
